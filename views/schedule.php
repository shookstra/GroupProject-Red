...
while($row = mysqli_fetch_array($result)) {
	...
	$num_sessions = ($end_time - $start_time)*2 - 1;
        ...
        $check_times_sql = "SELECT appTime
            from appointment
            where tutorID ='".$tutorID."' && appDate ='".$appDate."'
            group by tutorID";//query to pull any times the tutor may have reserved already on that day
        
        $check_times=mysqli_query($con,$check_times_sql);
        $testing = mysqli_fetch_array($check_times);
	...
	for($i = 0; $i <= $num_sessions; $i++){
                ...
                if(!empty($testing)){//checks if the $testing variable has anything in it
		if(strpos($timeStr, ".") !== false){// checks if $timestr has a "." in it
                    $timePrint = intval($timeStr) . ":30";//changes the ".5" to :30
                   if(!in_array($timePrint, $testing, true)) {//checks if the tutor already has an appointment at a time to disable the button
                    ...
                    if(!in_array($timeStr, $testing, true)) {
